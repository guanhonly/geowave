...
import org.locationtech.geowave.core.store.operations.BaseDeleter;
		...
		DataStore
{
					...
					if (delete) {
						...
						DeleteCallbackList<T, GeoWaveRow> delList = (DeleteCallbackList<T, GeoWaveRow>) callbackCache
								.getDeleteCallback(
										adapter,
										index);
						delList.AddCallback(new DeleteDuplicateCallback<T>(
								this,
								queryOptions));
	...
	public boolean delete(
		...
		if (((query == null) || (query instanceof EverythingQuery))) {
			...
			try (final CloseableIterator<?> dataIt = internalQuery(
					queryOptions,
					query,
					true,
					false)) {
